...
import requests # Import the requests library for fetching data from URLs
import io       # Import io for handling byte streams

# --- Konfigurasi Penting ---
MODEL_PATH = 'https://raw.githubusercontent.com/enricochandran/Tubes-Data-Science/b0630318d2eb2244049320373e725618c2826a17/model_naive_bayes.pkl'
DATA_PATH = 'https://raw.githubusercontent.com/enricochandran/Tubes-Data-Science/b0630318d2eb2244049320373e725618c2826a17/processed_heart_disease_data.csv'

# Kolom target Anda: 'num'
TARGET_COLUMN = 'num'
...
def load_model(url):
    """
    Loads the machine learning model from a given URL using requests and pickle.
    Uses st.cache_resource to cache the model, preventing re-download on every rerun.
    """
    try:
        ...
        return {"success": False, "message": f"Error fetching model from URL '{url}': {e}. Please check the URL and your internet connection."}
    except Exception as e:
        return {"success": False, "message": f"Error loading model from content: {e}"}

@st.cache_data
def load_data(url):
    """
    Loads the processed heart disease data from a given URL using requests and pandas.
    Uses st.cache_data to cache the DataFrame, preventing re-download on every rerun.
    """
    try:
        # Fetch the data CSV file from the URL
        response = requests.get(url)
        response.raise_for_status() # Raise an HTTPError for bad responses (4xx or 5xx)

        # Load the data from the text content of the response into a pandas DataFrame
        data = pd.read_csv(io.StringIO(response.text))
        if 'Unnamed: 0' in data.columns:
            data = data.drop(columns=['Unnamed: 0'])
        return {"success": True, "data": data}
    except requests.exceptions.RequestException as e:
        return {"success": False, "message": f"Error fetching data from URL '{url}': {e}. Please check the URL and your internet connection."}
    except pd.errors.EmptyDataError:
        return {"success": False, "message": f"Error: File '{url}' kosong. Pastikan ada data di dalam CSV."}
    except Exception as e:
        return {"success": False, "message": f"Error loading data CSV from content: {e}"}

def get_model_accuracy(model, data_df, target_col, dropped_cols_from_features):
    """
    Calculates the accuracy of the loaded model on a test subset of the data.
    Handles binary classification transformation for the target column.
    """
    ...
    # Convert original multi-class target to binary (0 = no disease, >0 = disease)
    ...
    try:
        ...
        # Check if stratification is possible (at least 2 samples in each class)
        ...
        # Convert raw predictions to binary (0 = no disease, >0 = disease)
...
def main():
    ...
    # Load model and data from GitHub URLs
    ...
    # Define required user inputs explicitly
    ...
    # Placeholders for text input fields
    placeholders = {
        'age': "Contoh: 45",
        'thalach': "Contoh: 150",
        'oldpeak': "Contoh: 1.5",
    ...
    # Calculate min/max values for tooltips
    min_max_values = {}
    ...
    # Explicitly define numeric and categorical features for consistent handling
    ...
    for feature in feature_names_for_model:
        with cols[col_idx]:
            if feature in processed_data_for_features.columns:
                ...
                # Format tooltip with min/max values
                ...
                if feature in CATEGORICAL_FEATURES:
                    ...
                    display_options = ["Pilih..."] # Default "Select..." option
                    value_options_map = {} # Map display values to actual numerical values

                    # Specific display options for better user experience
                    if feature == 'sex':
                    ...
                    else: # Fallback for other categorical features if any
                ...
                elif feature in NUMERIC_FEATURES: # Only for pure numeric features
                    ...
                    if user_val_str: # Only process if input is not empty
                        try:
                            ...
                            # Warn if value is outside historical range
                            min_val = processed_data_for_features[feature].min()
                            max_val = processed_data_for_features[feature].max()
                            ...
                            user_input[feature] = None # Set to None if invalid input
                    else:
                        user_input[feature] = None # Set to None if input is empty

                else: # Fallback if feature is not categorized (shouldn't happen with explicit lists)
        ...
        col_idx = (col_idx + 1) % num_cols # Move to the next column for input fields

    st.markdown("<br>", unsafe_allow_html=True) # Add some vertical space
    input_df = pd.DataFrame([user_input])

    # Replace None with NaN for proper pandas handling of missing values
    input_df = input_df.replace({None: np.nan})

    # Disable predict button if any required input is missing
    ...
    if predict_button:
        try:
            # Ensure input data types match the original data for the model
            for feature in feature_names_for_model:
            ...
            # Ensure the order of columns in input_df matches what the model expects
            input_df = input_df[feature_names_for_model]

            prediction_raw = model.predict(input_df)

            # Convert raw prediction to binary: 0 for no disease, 1 for disease
